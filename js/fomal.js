...
$.ajax({
  ...
  data: {
    key: 'WPXBZ-PJVWZ-5C7XG-T4WNK-MEZAS-NWBS4',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(114.92085, 25.81751, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
  ...
  try {
    ...
    document.getElementById("welcome-info").innerHTML =
      `<b><center>🎉 欢迎信息 🎉</center>&emsp;&emsp;欢迎来自 <span style="color:var(--theme-color)">${pos}</span> 的小伙伴，${timeChange}您现在距离站长约 <span style="color:var(--theme-color)">${dist}</span> 公里，当前的IP地址为： <span style="color:var(--theme-color)">${ip}</span> <br/> ${posdesc}</br>`;
  } catch (err) {
  ...
  (function ($) {
    $.fn.nekoScroll = function (option) {
      var defaultSetting = {
        ...
        borderRadius: 5 + 'px',
        right: 55.6 + 'px',
        nekoImg: "https://gitee.com/winterdev/images/raw/master/tangyuan.webp",
        hoverMsg: "我家汤圆~",
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("06/05/2024 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> </div>`
      : `<div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> </div>`),
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://gitee.com/winterdev/images/raw/master/home_bg2.webp);
    --darkmode-bg: url(https://gitee.com/winterdev/images/raw/master/home_bg_darkmode.webp);
    --mobileday-bg: url(https://gitee.com/winterdev/images/raw/master/home_bg2.webp);
    --mobilenight-bg: url(https://gitee.com/winterdev/images/raw/master/home_bg_darkmode.webp);
  }`;
}
// 切换背景主函数
function changeBg(s) {
